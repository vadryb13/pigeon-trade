...
async def get_run(run_id: str) -> dict[str, Any] | None:
    ...
    async with _async_session_factory() as db:
        ...
        run = await store.get_run(uuid.UUID(run_id))
        if run is None:
            return None
        hyps = await store.list_hypotheses_by_run(uuid.UUID(run_id))
        return {
            "id": str(run.id),
            "goal": run.goal,
...
async def compare_runs(
    ...
    async with _async_session_factory() as db:
        store = RegistryStore(db)
        run_a = await store.get_run(uuid.UUID(run_id_a))
        run_b = await store.get_run(uuid.UUID(run_id_b))
...
async def search_similar_hypotheses(
    ...
    Returns:
        ...
        если БД недоступна или совпадений нет.
    """
    try:
